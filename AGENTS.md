...
## Influish Visual Direction

- Keep recent reel insights directly below the banner and Powered by Meta lockup for every Instagram-connected user, whether or not the user has an active subscription.
- The Home area from the top of the app through Powered by Meta uses a lavender canvas.
- Aim for a premium, colourful, breathable, new-age creator experience with warm hospitality-app polish: fewer competing borders, stronger spacing, soft depth, tactile 3D tool artwork, and clear hierarchy.
- Quick Access and Auto-DM tools should use real 3D raster artwork rather than flat icon-only tiles.
- The bottom navigation uses restrained glassmorphism with translucency, blur, a soft inner highlight, and clear active states.
- Preserve the existing five-tab structure, three prototype user states, and all core interactions while applying this visual direction.
